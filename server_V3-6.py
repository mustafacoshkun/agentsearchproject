...
# langdetect varsayılan olarak deterministik değildir (rastgele seed kullanır),
# aynı sorgu farklı çalıştırmalarda farklı sonuç verebilir. Seed sabitleniyor.
...
class Config:
    ...
    # Dil algılama: langdetect kısa metinlerde (özellikle tek kelime) güvenilir değil.
    # Bu eşiğin altındaki sorgular dil kontrolünden muaf tutulur.
    MIN_LANG_DETECT_LEN = 12
...
async def search_endpoint(
    ...
    try:
        ...
        # Language detection
        # langdetect istatistiksel bir sınıflandırıcıdır ve kısa metinlerde (tek kelime,
        # "matematik", "5.sınıf" gibi 1-2 kelimelik sorgularda) güvenilmez sonuçlar üretir —
        # sıkça tr/en dışında rastgele bir dil tahmin edip geçerli sorguları reddeder.
        # Bu yüzden kısa sorgularda dil kontrolü tamamen atlanır.
        query_stripped = query.strip()
        if len(query_stripped) >= Config.MIN_LANG_DETECT_LEN:
            try:
                ...
                # Algılama başarısız olduysa sorguyu reddetmek yerine devam ettiriyoruz —
                # belirsizlik güvenlik riski değil, sadece kısa/gürültülü bir metin demektir.
                print("[LANG] ⚠️ Dil algılanamadı, sorguya izin veriliyor")
        else:
            print(f"[LANG] Sorgu kısa ({len(query_stripped)} karakter), dil kontrolü atlandı")
