...
class Config:
    ...
    VLLM_MODEL = "./local_model"  # V9'daki --served-model-name ile birebir aynı olmalı
    VLLM_TIMEOUT = 8  # H100 bazen kapalı olabiliyor; hızlı vazgeçip Ollama'ya düş
    VLLM_SYSTEM_PROMPT = (
        "Sen MEBİ platformunun eğitim asistanı KANKA'sın. Öğrencilere okul dersleri, LGS ve YKS "
        "hazırlığında samimi ve insancıl bir dille rehberlik edersin. Sana verilen Durum bilgisine "
        "göre en fazla 2 cümlelik kısa bir yanıt verirsin. Bulunan içerikler ekranda ayrıca listelendiği "
        "için onları tekrar yazmazsın. Ders sorularını doğrudan cevaplamaz, ilgili içeriğe yönlendirirsin. "
        "Argo kullanmaz, kışkırtmaya karşılık vermezsin. Kalıp cümleleri tekrarlamazsın."
    )
...
class TextUtils:
    ...
    _SMALLTALK_STOPWORDS = {"kanka", "abi", "abla", "hocam", "dostum", "canım", "ya", "be",
                            "ben", "sen", "siz", "bana", "sana", "size", "benim", "senin"}

    # Kimlik soruları — arama motoruna hiç gitmemeli, model sabit MEB cevabı vermeli
    _KIMLIK_PATTERN = re.compile(
        r"seni kim (yaptı|yazdı|geliştirdi|oluşturdu|kurdu|üretti)|hangi (şirket|firma)\s*"
        r"(yaptı|geliştirdi|tarafından)|sen (bir )?yapay zeka mısın|robot musun|"
        r"kim tarafından (yapıldın|geliştirildin|yazıldın)",
        re.IGNORECASE
    )

    # Saf ruh hali ifadeleri — ders/konu adı geçmeyen, tek başına duygu durumu
    _DUYGUSAL_SAF_PATTERN = re.compile(
        r"(sıkıl\w*|bıktım|yoruldum|moralim|stresli\w*|kaygılı\w*|panikte|üzgün\w*|"
        r"mutsuz\w*|keyifsiz\w*)",
        re.IGNORECASE
    )
    _DERS_KELIME_PATTERN = re.compile(
        r"\b(ders|konu|sınav|yazılı|test|ünite|matematik|fizik|kimya|biyoloji|tarih|"
        r"coğrafya|felsefe|edebiyat|türkçe|ingilizce|geometri)\b",
        re.IGNORECASE
    )

    @staticmethod
    def is_kimlik_sorusu(query: str) -> bool:
        """'Seni kim yaptı', 'yapay zeka mısın' gibi kimlik sorularını yakalar."""
        return bool(TextUtils._KIMLIK_PATTERN.search(query))

    @staticmethod
    def is_duygusal_saf(query: str) -> bool:
        """Ders/konu adı geçmeyen, tek başına ruh hali ifadesini yakalar
        (örn. 'canım çok sıkılıyor bugün'). Ders adı geçen karışık cümleler
        ('matematik dersinde sıkılıyorum') burada YAKALANMAZ, normal aramaya girer."""
        if not TextUtils._DUYGUSAL_SAF_PATTERN.search(query):
            return False
        return not TextUtils._DERS_KELIME_PATTERN.search(query)
...
class LLMEngine:
    ...
    @staticmethod
    def build_prompt(durum: str, query: str, ui_class: Optional[str] = None,
                     ders_adi: Optional[str] = None, results: Optional[List[dict]] = None) -> str:
        """Sorgu + durumu modelin eğitildiği formata çevirir (Colab eğitim formatıyla birebir aynı)."""
        satirlar = [f"Durum: {durum}"]

        meta = []
        if ui_class: meta.append(f"Sınıf: {ui_class}")
        if ders_adi: meta.append(f"Ders: {ders_adi}")

        konu = None
        if durum == "BULUNDU" and results:
            konu_tam = (results[0].get("text") or "").strip()
            if konu_tam:
                konu = konu_tam.split("->")[-1].strip()
        if konu:
            meta.append(f"Konu: {konu}")

        if meta:
            satirlar.append(" | ".join(meta))

        satirlar.append(f"Soru: {query.strip()}")
        return "\n".join(satirlar)
    ...
    async def generate(self, query: str, results: List[dict], durum: str,
                       ui_class: Optional[str] = None, ders_adi: Optional[str] = None) -> dict:
        """
        ...
        """
        prompt = self.build_prompt(durum, query, ui_class, ders_adi, results)
        start = time.time()
...
async def search_endpoint(
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        is_smalltalk = await loop.run_in_executor(None, TextUtils.is_blacklisted, query)
        is_kimlik = await loop.run_in_executor(None, TextUtils.is_kimlik_sorusu, query)
        is_duygusal_saf = await loop.run_in_executor(None, TextUtils.is_duygusal_saf, query)

        if is_smalltalk or is_kimlik or is_duygusal_saf:
            sebep = "kimlik sorusu" if is_kimlik else ("duygusal/ruh hali" if is_duygusal_saf else "sohbet/selamlaşma")
            print(f"[INFO] 💬 Saf {sebep} sorgusu → arama atlanıp doğrudan LLM'e yönlendiriliyor")

            llm_result = await llm_engine.generate(query, [], "ANLAMSIZ", ui_class, ders_adi)
        ...
        if not results:
            ...
            # Bağlam yok → model Durum: BULUNAMADI alır
            llm_result = await llm_engine.generate(query, [], "BULUNAMADI", ui_class, ders_adi)
        ...
        # LLM: Sorgu + getirilen ilk içerik birleştirilip modele verilir
        llm_result = await llm_engine.generate(query, results, "BULUNDU", ui_class, ders_adi)
